...
def get_today():
    """Always return current date (fix Streamlit cloud freeze issue)"""
    return datetime.now().strftime("%Y-%m-%d")


# =============================
# 2. HRP-CVaR FUNCTIONS
# =============================
...
def get_rec_bisection_cvar(returns, sort_ix):
    ...
    while len(items) > 0:
        ...
        for i in range(0, len(items), 2):
            ...
            cv0 = calc_cvar(returns[c0])
            cv1 = calc_cvar(returns[c1])
...
def get_mvo_weights(train_returns):
    ...
    try:
        ...
        mu = expected_returns.mean_historical_return(
            train_returns, returns_data=True
        )
...
def update_dataset():
    ...
    g_val = (
        gold_raw["GC=F"].iloc[:, 0]
        if isinstance(gold_raw["GC=F"], pd.DataFrame)
        else gold_raw["GC=F"]
    )

    f_val = (
        gold_raw["USDVND=X"].iloc[:, 0]
        if isinstance(gold_raw["USDVND=X"], pd.DataFrame)
        else gold_raw["USDVND=X"]
    )
